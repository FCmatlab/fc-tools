function colors = selectColors(N,varargin)
%  FUNCTION fc_tools.graphics.selectColors
%    returns N colors that are maximally perceptually distinct without using
%    the Image Processing Toolbox.
%    The N parameter can be followed by key/value pairs to specify options. 
%    The keys will be the strings:
%     * 'background':
%       The N colors selected will be as far as possible from the colors 
%       specified by this options as a n-by-3 RGB colors array.
%       Default is [1 1 1; 0 0 0; 0.8 0.8 0.8;1,0,1]
%     * 'func':
%       To specify an other function for converting RGB colors to LAB colors.
%       Default is local RGB2LAB function.
%
%    Inspired by the function select_colors (or distinguishable_colors) of
%    Timothy E. Holy which uses the Image Processing Toolbox.
%    see https://fr.mathworks.com/matlabcentral/fileexchange/29702-generate-maximally-perceptually-distinct-colors
%
% SAMPLES
%   colors=fc_tools.graphics.selectColors(10);
%   colors=fc_tools.graphics.selectColors(10,'background',[0 0 0;1 1 1]);
%
% <COPYRIGHT>
  p = inputParser;
  ...
  [R,G,B] = ndgrid(x,x,x);
  rgb = [R(:) G(:) B(:)];
  if (N > size(rgb,1)/3)  % size(rgb,1)/3==9000 !
...
%    RGB2LAB is given to avoid using if the rgb2lab fonction of the Image 
%    Processing Toolbox.
